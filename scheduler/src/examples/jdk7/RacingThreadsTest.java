 ...
 * <ul>
 * <li>
 * Extend RacingThreadsTest class in order to provide the test specific
 * variables and/or code, e.g., <br>
 * public class MyRacingThreadsTest extends RacingThreadsTest
 * <li>
 * Use "new MyRacingThreadsTest(name, n_threads, n_loops, n_secs)" to create
 * your test with the specified name and the specified number of WorkerThreads
 * that execute the test code in parallel up to n_loops iterations or n_secs
 * seconds.
 * <li>
 * Use "new DriverThread(test)" to create the test DriverThread that manages all
 * the WorkerThreads. The DriverThread class can be extended to provide test
 * specific code and/or variables. However, that is typically done in your
 * test's subclass.
 * <li>
 * Use "new WorkerThread(workerNum, test)" to create WorkerThread-workerNum that
 * executes the test code. The WorkerThread class can be extended to provide
 * test thread specific code and/or variables.
 * <li>
 * Use "RacingThreadsTest.runTest(driver, workers)" to run the test. If the test
 * fails, then a RuntimeException is thrown.
 * </ul>
 *
 * The RacingThreadsTest class provides many methods that can be overridden in
 * order to provide test specific semantics at each identified test execution
 * point. At a minimum, your test's subclass needs to override the
 * "void executeRace(WorkerThread)" method in order to exercise your race
 * condition and it needs to override the "void checkRaceResults(DriverThread)"
 * method in order to check the results of the race. Your checkRaceResults()
 * method should call the "int incAndGetFailCnt()" method when it detects a
 * failure. It can also call the "void unexpectedException(Thread, Exception)"
 * method if it detects an unexpected exception; this will cause an error
 * message to be output and the failure count to be incremented. When the
 * RacingThreadsTest.runTest() method is done running the races, if there is a
 * non-zero failure count, then a RuntimeException will be thrown.
 * <p>
 * The RacingThreadsTest class uses three internal barriers to coordinate
 * actions between the DriverThread and the WorkerThreads. These barriers should
 * not be managed or used by your test's subclass and are only mentioned here to
 * provide clarity about interactions between the DriverThread and the
 * WorkerThreads. The following transaction diagram shows when the different
 * RacingThreadsTest methods are called relative to the different barriers:
 *
 * <pre>
 ...
 * <ul>
 * <li>
 * After the DriverThread starts the WorkerThreads, the DriverThread and the
 * WorkerThreads are running in parallel until the startBarrier is reached.
 * <li>
 * After the WorkerThreads leave the startBarrier, they are running the code in
 * executeRace() in parallel which is the whole point of this class.
 * <li>
 * The DriverThread heads straight to the finishBarrier and waits for the
 * WorkerThreads to get there.
 * <li>
 * After the DriverThread leaves the finishBarrier, it checks the results of the
 * race.
 * <li>
 * The WorkerThreads head straight to the resetBarrier and wait for the
 * DriverThread to get there.
 * <li>
 * If this is not the last race, then after the DriverThread and WorkerThreads
 * leave the resetBarrier, the DriverThread and the WorkerThreads are running in
 * parallel until the startBarrier is reached.
 * <li>
 * If this is the last race, then after the DriverThread and WorkerThreads leave
 * the resetBarrier, the DriverThread and the WorkerThreads are running in
 * parallel as each WorkerThread ends.
 * <li>
 * The DriverThread waits for the WorkerThreads to end and then it ends
 * </ul>
 *
 * Once the DriverThread has ended, the RacingThreadsTest.runTest() method
 * checks the failure count. If there were no failures, then a "Test PASSed"
...
public class RacingThreadsTest {
    ...
    /**
     * Creates a test with the specified name and the specified number of
     * WorkerThreads that execute the test code in parallel up to n_loops
     * iterations or n_secs seconds. The RacingThreadsTest class is extended in
     * order to provide the test specific variables and/or code.
     * 
     * @param name
     ...
     * @param n_loops
     *            the maximum number of test iterations
     * @param n_secs
     *            the maximum number of seconds to execute the test loop
     */
    ...
    protected static final AtomicInteger failCnt = new AtomicInteger();
    ...
    private static boolean verbose = Boolean
            .getBoolean("RacingThreadsTest.verbose");
    ...
    /**
     * Run the test with the specified DriverThread and the specified
     * WorkerThreads.
     * 
     * @param driver
     *            the DriverThread for running the test
     * @param workers
     *            the WorkerThreads for executing the race
    ...
    /**
     * Helper method for reporting an unexpected Exception and calling
     * incAndGetFailCnt();
     * 
     * @param t
     *            the Thread that caught the exception
     * @param e
    ...
    /**
     * Initialize 1-time items for the DriverThread. Called by the DriverThread
     * before WorkerThreads are started.
     * 
     * @param dt
     *            the DriverThread
     */
    ...
    /**
     * Initialize 1-time items for a WorkerThread. Called by a WorkerThread
     * after oneTimeDriverInit() and before the WorkerThread checks in with
     * startBarrier. May execute in parallel with perRaceDriverInit() or with
     * another WorkerThread's oneTimeWorkerInit() call or another WorkerThread's
     * perRaceWorkerInit() call.
     * 
     * @param wt
     *            the WorkerThread
     */
     ...
     * DriverThread before it checks in with startBarrier. May execute in
     * parallel with oneTimeWorkerInit() and perRaceWorkerInit() calls. After
     * any race except for the last race, this method may execute in parallel
     * with perRaceWorkerEpilog().
     * 
     * @param dt
     *            the DriverThread
     */
    ...
    /**
     * Initialize per-race items for a WorkerThread. Called by each WorkerThread
     * before it checks in with startBarrier. On the first call, this method may
     * execute in parallel with another WorkerThread's oneTimeWorkerInit() call.
     * On any call, this method may execute in parallel with perRaceDriverInit()
     * or another WorkerThread's perRaceWorkerInit() call. After any race except
     * for the last race, this method may execute in parallel with
     * perRaceDriverEpilog() or another WorkerThread's perRaceWorkerEpilog()
     * call.
     * 
     * @param wt
     *            the WorkerThread
    ...
    /**
     * Execute the race in a WorkerThread. Called by each WorkerThread after it
     * has been released from startBarrier.
     * 
     * @param wt
     *            the WorkerThread
     */
    ...
    /**
     * Check race results in the DriverThread. Called by the DriverThread after
     * it has been released from finishBarrier and before the DriverThread
     * checks in with resetBarrier.
     * 
     * @param dt
     *            the DriverThread
     */
    ...
    /**
     * Handle end-of-race items for the DriverThread. Called by the DriverThread
     * after it has been released from resetBarrier and before the DriverThread
     * checks in again with startBarrier. Can execute in parallel with
     * perRaceWorkerEpilog(). If this is not the last race, can execute in
     * parallel with perRaceWorkerInit(). If this is the last race, can execute
     * in parallel with oneTimeWorkerEpilog().
     * 
     * @param dt
     *            the DriverThread
     */
    ...
    /**
     * Handle end-of-race items for a WorkerThread. Called by each WorkerThread
     * after it has been released from resetBarrier and before the WorkerThread
     * checks in again with startBarrier. Can execute in parallel with
     * perRaceDriverEpilog() or another WorkerThread's perRaceWorkerEpilog()
     * call. If this is not the last race, can execute in parallel with
     * perRaceDriverInit(), or another WorkerThread's perRaceWorkerInit() call.
     * If this is the last race, can execute in parallel with another
    ...
    /**
     * Handle end-of-test items for a WorkerThread. Called by each WorkerThread
     * after it has detected that all races are done and before
     * oneTimeDriverEpilog() is called. Can execute in parallel with
     * perRaceDriverEpilog(), with another WorkerThread's perRaceWorkerEpilog()
     * call or with another WorkerThread's oneTimeWorkerEpilog() call.
     * 
     * @param wt
     *            the WorkerThread
     */
    ...
    /**
     * Handle end-of-test items for the DriverThread. Called by the DriverThread
     * after all the WorkerThreads have called oneTimeWorkerEpilog().
     * 
     * @param dt
     *            the DriverThread
     */
    ...
    public static class DriverThread extends Thread {
        ...
        /**
         * Create the test DriverThread that manages all the WorkerThreads. The
         * DriverThread class can be extended to provide test specific variables
         * and/or code. However, that is typically done in the subclass of
         * RacingThreadsTest.
         * 
        ...
        private void run(WorkerThread[] workers) {
            ...
            System.out.println(getName() + ": # WorkerThreads: "
                    + test.N_THREADS);
            ...
            for (; !test.getDone() && test.getLoopCnt() < test.N_LOOPS; test
                    .incAndGetLoopCnt()) {

                if (test.getVerbose()
                        && (test.N_LOOPS < 10 || (test.getLoopCnt() % (test.N_LOOPS / 10)) == 0)) {
            ...
            if (test.getLoopCnt() < test.N_LOOPS) {
                System.out.println(getName() + ": race stopped @ "
                        + test.N_SECS + " seconds.");
            }
    ...
    public static class WorkerThread extends Thread {
        ...
        /**
         * Creates WorkerThread-N that executes the test code. The WorkerThread
         * class can be extended to provide test thread specific variables
         * and/or code.
         * 
         * @param workerNum
         *            the number for the new WorkerThread
         * @parameter test the RacingThreadsTest being run
         */
