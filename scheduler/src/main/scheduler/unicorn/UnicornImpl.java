...
//		List<Pattern> patterns = PatternInit.initConsisitencyPatterns();
//		System.out.println(patterns);
//		
//		Result failResult = null;
//		while (failResult == null) {
//			Result result = Replay.run("reorder.ReorderTest");
//			if (!result.success) {
//				failResult = result;
//			}
//		}
//		FileParser.toXML(failResult.mixed, false, "C:\\Users\\’≈∂∞µœ\\Desktop\\mixed.xml");
//		
//		failResult.matchPatterns(patterns);
//		System.out.println(failResult.matchedPatterns);
//	}
//}
...
public class UnicornImpl {
	...
	public static final String path = "UNICORN execution\\";
	public static final String entry = "org.apache.commons.dbcp.Dbcp271";
...
//					getFailedCount(p), totalFailed, getSuccessCount(p), totalSuccess, sus));
		...
		for (Pattern p : rank) {
			...
			if (suspeClone.get(p) > 0.2) {
				suspB2++;
			}
			if (suspeClone.get(p) > 0.5) {
				suspB5++;
			}
		}
		...
		output += "\n > 0.2: " + suspB2 + ", > 0.5: " + suspB5 + ", total: " + rank.size();
